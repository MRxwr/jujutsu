...
if( isset($_GET["hide"]) && !empty($_GET["hide"]) ){
	if( updateDB('sessions',array('hidden'=> '2'),"`id` = '{$_GET["hide"]}'") ){
		header("LOCATION: ?v=Sessions");
	}
}

if( isset($_GET["show"]) && !empty($_GET["show"]) ){
	if( updateDB('sessions',array('hidden'=> '1'),"`id` = '{$_GET["show"]}'") ){
		header("LOCATION: ?v=Sessions");
	}
}

if( isset($_GET["delId"]) && !empty($_GET["delId"]) ){
	if( updateDB('sessions',array('status'=> '1'),"`id` = '{$_GET["delId"]}'") ){
		header("LOCATION: ?v=Sessions");
	}
}
...
if( isset($_POST["updateRank"]) ){
	for( $i = 0; $i < sizeof($_POST["rank"]); $i++){
		updateDB('sessions',array("rank"=>$_POST["rank"][$i]),"`id` = '{$_POST["id"][$i]}'");
	}
	header("LOCATION: ?v=Sessions");
}
...
if( isset($_POST["enTitle"]) ){
	...
	if ( $id == 0 ){
		...
		if( updateDB('sessions', $_POST, "`id` = '{$id}'") ){
			header("LOCATION: ?v=Sessions");
		}else{
		?>
		<script>
...
<div class="panel-heading">
<div class="pull-left">
<h6 class="panel-title txt-dark"><?php echo direction("List of Sessions","قائمة الكلاسات") ?></h6>
</div>
<div class="clearfix"></div>
</div>
<div class="panel-wrapper collapse in">
...
<div class="table-responsive">
	<table class="table display responsive product-overview mb-30" id="myTable">
		...
		<tbody>
		<?php 
		if( $sessions = selectDB('sessions',"`status` = '0' ORDER BY `rank` ASC") ){
			for( $i = 0; $i < sizeof($sessions); $i++ ){
				$counter = $i + 1;
				if ( $sessions[$i]["hidden"] == 2 ){
					$icon = "fa fa-eye";
					$link = "?v={$_GET["v"]}&show={$sessions[$i]["id"]}";
				...
				<input name="id[]" class="form-control" type="hidden" value="<?php echo $sessions[$i]["id"] ?>">
				</td>
				...
				<td class="text-nowrap">
					...
					<a href="<?php echo "?v={$_GET["v"]}&delId={$sessions[$i]["id"]}" ?>" data-toggle="tooltip" data-original-title="<?php echo direction("Delete","حذف") ?>"><i class="fa fa-close text-danger"></i>
					</a>
				<div style="display:none"><label id="hidden<?php echo $sessions[$i]["id"]?>"><?php echo $sessions[$i]["hidden"] ?></label></div>		
				</td>
				</tr>
				<?php
			}
